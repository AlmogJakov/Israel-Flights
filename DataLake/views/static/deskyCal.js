...
function initDeskyCalendarAll() {
  if (Object.keys(deskyOpts).length == 0) {
    console.log("Init deskyOpts Object beforer using this function");
    return;
  }
  ...
  Array.from(Object.keys(deskyOpts)).forEach(function (k) {
    ...
    if (!deskyOpts[k]["any_date"]) {
      ...
      newInput.value =
        inDate.getFullYear() +
        "-" +
        String(parseInt(inDate.getMonth()) + 1).padStart(2, "0") +
        "-" +
...
function insertSchema(parent, id) {
  ...
  if (deskyOpts[id].any_date) {
    el +=
      "			<span data-day='calsel_anyDate_" +
      id +
      "' class='curr-month cal-sel-day' onClick='dayClick(this)'>Any Date</span>";
  }
  // Close right
  el += "		</div>";
  el += "	</div>";
  ...
  for (let i = 1; i <= 21; i++) {
    ...
    if (tdate.getDay() == 0)
      dows += "<span>" + tdate.toLocaleString("default", { weekday: "long" }).charAt(0) + "</span>";
...
function drawCalSel(calParent, inputId, curr_year, curr_month) {
  ...
  for (let i = 1; i <= numLast; i++) {
    ...
    /// disabled after
    if (tdate.getTime() > deskyOpts[inputId].disabled_after && deskyOpts[inputId].disabled_after != null)
      extraClass = "disabled";

    out +=
      "<span data-day='" +
      tid +
      "' class='curr-month cal-sel-day " +
      extraClass +
...
function dayClick(el) {
  ...
  if (idA[1] == "anyDate") {
    ...
    input.value =
      ...
      "-" +
      String(parseInt(centerDate.getMonth()) + 1).padStart(2, "0") +
      "-" +
      String(parseInt(centerDate.getDate())).padStart(2, "0");
  }
